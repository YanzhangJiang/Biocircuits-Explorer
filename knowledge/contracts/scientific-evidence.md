...
but unsupported claims. This contract keeps the boundary explicit and is verified
against `f9c65a5`.
...
Use when the engine, corpus, dataset identity, or required assumption is missing.
Return an error, abstention, or `unknown`. Do not substitute a plausible network,
zero, or stale number.
...
show that the candidate was freshly simulated for the current request. The Design
Agent must pass a selected prior through the live compute tool before presenting
it as a verified current-session answer.

### Current computation

A current engine result records the concrete network, inputs, outputs, parameters
or scan policy, algorithm/version, and computed curve, surface, phenotype, or ROP
result. In the conversational layer, `simulate`/`simulate_2d` create this class of
evidence. If the engine reports offline, the agent must abstain rather than reuse
the prior as an answer.

A computation verifies only the declared run. It does not prove robustness over
an unscanned region, global optimality, biological implementability, or a theorem.
...
proof. A compatibility alias such as `recommended` must equal the verified list,
not the screened list.
...
not certify absence for non-trivial cells (`src/periodic_table/candidate_search.py`).
...
- warnings, invalid/non-converged points, and cancellation/partial status;
...
Software output becomes a publication claim only after the manuscript repository
records a claim entry that pins:
...
- “robust” without the parameter region and metric;
...
Prefer scoped wording such as “the current engine run satisfied…,” “a witness
exists within grammar G and bounds B,” or “no candidate was found by bounded
search S.”
...
- `webapp/test/designability_spec_contract.jl` checks proxy/verified separation,
  evidence grades, hard constraints, and certificate semantics.
