...
# SBML SId must match [A-Za-z_][A-Za-z0-9_]*. Species names produced by the
# reaction parser already satisfy this; only the model id (from a free-form
...
species). Each reaction's `kd` is emitted both as a global parameter
`Kd_<reactionId>` and inside the reaction's kineticLaw, so standard tools and
our own importer can both recover it.
"""
function network_ir_to_sbml(ir::NetworkIR)
    ...
    model_id = _sanitize_sid(ir.label; fallback = "biocircuits_model")
    label_attr = isempty(ir.label) ? "" : " name=\"$(_xml_escape(ir.label))\""
    ...
    for name in ordered_species
        ...
        println(io, "      <species id=\"$(name)\" name=\"$(_xml_escape(name))\" compartment=\"default\"$init_attr " *
    ...
    for pd in ir.parameter_distributions
        if pd.kind == :point && pd.value !== nothing
            println(io, "      <parameter id=\"$(_sanitize_sid(pd.symbol))\" value=\"$(pd.value)\" constant=\"true\"/>")
        else
    ...
    for (i, rx) in enumerate(ir.reactions)
        ...
        println(io, "      <reaction id=\"r$(i)\" reversible=\"$rev\" compartment=\"default\">")
        if !isempty(lhs)
            println(io, "        <listOfReactants>")
            for (sp, c) in lhs
                println(io, "          <speciesReference species=\"$(sp)\" stoichiometry=\"$(c)\" constant=\"true\"/>")
            end
            println(io, "        </listOfReactants>")
        end
        if !isempty(rhs)
...
function sbml_to_network_ir(xml::AbstractString)
    ...
    label = _sbml_attr(model, "name", _sbml_attr(model, "id", ""))
        ...
        for p in _sbml_children(param_list, "parameter")
            ...
            if !isempty(id) && v !== nothing
                ...
                if !occursin(r"^Kd_r\d+$", id)
                    push!(param_dists, ParameterDistribution(symbol = id, kind = :point, value = v))
                end
            end
        end
    end
    ...
    if rx_list !== nothing
        for (i, rx) in enumerate(_sbml_children(rx_list, "reaction"))
            ...
            push!(reaction_decls, ReactionDecl(formula = formula, kd = kd, reversible = reversible))
        end
    end
