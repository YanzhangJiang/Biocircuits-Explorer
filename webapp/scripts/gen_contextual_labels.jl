...
const EVAL_VERSION = "bne-contextual-eval/v0.1.0"
...
nslice = 0; nok = 0; nrepro = 0; t0 = time(); outer = false
for (nid, e) in nets
    outer && break
        ...
        for out in outs
            ...
            try
                ...
                row = Dict("network_id" => nid, "rules" => e["rules"], "inputs" => [a, b],
                           ...
                           "per_context" => r["per_context"], "evaluator_version" => EVAL_VERSION)
...
close(io)
println("LABELS=$nok reprogrammable=$nrepro slices_seen=$nslice wall=$(round(time()-t0,digits=1))s")
println("GENCTX.DONE")
