...
const EVAL_VERSION = "bne-analog-eval/v0.1.0"
...
io = open(outpath, "w")
nslice = 0; nok = 0; nerr = 0; t0 = time(); outer = false
for (nid, e) in nets
    outer && break
...
close(io)
dt = time() - t0
println("LABELS=$nok errors=$nerr slices_seen=$nslice wall=$(round(dt,digits=1))s rate=$(round(nok/max(dt,1e-9),digits=2)) slices/s")
