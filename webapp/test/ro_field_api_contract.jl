...
function _ro_field_contract_fixture(name)
    path = joinpath(@__DIR__, "..", "..", "tests", "fixtures", "ro_field", name)
    return Backend._materialize(JSON3.read(read(path, String)))
end

function _ro_field_scaled_exact_payload(scale::Float64)
    document = _ro_field_contract_fixture("exact-cell-complex.json")
    payload = Dict{String,Any}(
        key => deepcopy(document[key])
        for key in ("schema_version", "domain", "outputs", "component_order", "data")
    )
    for axis in payload["domain"]["axes"]
        axis["bounds"]["lower"] *= scale
        axis["bounds"]["upper"] *= scale
    end
    for cell in payload["data"]["cells"]
        for point in cell["vertices"], coordinate in eachindex(point)
            point[coordinate] *= scale
        end
        cell["area"] *= scale^2
    end
    for facet in payload["data"]["facets"]
        for point in facet["endpoints"], coordinate in eachindex(point)
            point[coordinate] *= scale
        end
        facet["offset"] *= scale
    end
    for stratum in payload["data"]["singular_strata"]
        for point in stratum["vertices"], coordinate in eachindex(point)
            point[coordinate] *= scale
        end
    end
    return payload
...
function _ro_field_test_exact_facet(facet_id, kind, endpoints,
                                    incident_cell_ids, domain_side)
    ...
    return Dict{String,Any}(
        ...
        "kind" => kind,
        "endpoints" => endpoints,
        "incident_cell_ids" => Any[incident_cell_ids...],
        "singular_stratum_ids" => Any[],
        "normal" => normal,
        "offset" => -(normal[1] * endpoints[1][1] +
...
function _ro_field_striped_exact_payload(; count::Int=16,
                                         interface_delta::Float64=0.0)
    payload = _ro_field_scaled_exact_payload(1.0)
    for axis in payload["domain"]["axes"]
        axis["bounds"] = Dict{String,Any}("lower" => -2.0, "upper" => 2.0)
    end
    nominal_width = 4.0 / count
    cells = Any[]
    for index in 1:count
        nominal_left = -2.0 + (index - 1) * nominal_width
        nominal_right = -2.0 + index * nominal_width
        left = index == 1 ? -2.0 : nominal_left - interface_delta / 2
        right = index == count ? 2.0 : nominal_right + interface_delta / 2
        cell_id = "cell-$index"
        source_id = "regime-$index"
        ...
        push!(cells, Dict{String,Any}(
            ...
            "status" => "regular",
            "vertices" => Any[
                Any[left, -2.0], Any[right, -2.0],
                Any[right, 2.0], Any[left, 2.0],
            ],
            "area" => nominal_width * 4.0,
            "source_regime_ids" => Any[source_id],
            "label_order" => Any[label_id],
            "affine_labels" => Any[Dict{String,Any}(
                "label_id" => label_id,
                "source_regime_ids" => Any[source_id],
                "output_offset" => Any[0.0],
                "reaction_order_matrix" => Any[Any[0.0, 0.0]],
            )],
            "set_valued" => false,
        ))
    end
    ...
    for (index, cell) in enumerate(cells)
        left = cell["vertices"][1][1]
        right = cell["vertices"][2][1]
        push!(facets, _ro_field_test_exact_facet(
            "bottom-$index", "domain_boundary",
            Any[Any[left, -2.0], Any[right, -2.0]],
            [cell["cell_id"]], "axis2_lower"))
        push!(facets, _ro_field_test_exact_facet(
            "top-$index", "domain_boundary",
            Any[Any[left, 2.0], Any[right, 2.0]],
            [cell["cell_id"]], "axis2_upper"))
    end
    push!(facets, _ro_field_test_exact_facet(
        "outer-left", "domain_boundary",
        Any[Any[-2.0, -2.0], Any[-2.0, 2.0]],
        [first(cells)["cell_id"]], "axis1_lower"))
    push!(facets, _ro_field_test_exact_facet(
        "outer-right", "domain_boundary",
        Any[Any[2.0, -2.0], Any[2.0, 2.0]],
        [last(cells)["cell_id"]], "axis1_upper"))
    for index in 1:(count - 1)
        boundary = -2.0 + index * nominal_width
        push!(facets, _ro_field_test_exact_facet(
            "transition-$index", "regime_transition",
            Any[Any[boundary, -2.0], Any[boundary, 2.0]],
            [cells[index]["cell_id"], cells[index + 1]["cell_id"]], nothing))
    end
    payload["data"] = Dict{String,Any}(
        "coefficient_encoding" => "float64",
        "source_candidate_regime_count" => count,
        "regular_candidate_regime_count" => count,
        "cell_order" => Any[getindex(cell, "cell_id") for cell in cells],
        "cells" => cells,
        "facet_order" => Any[getindex(facet, "facet_id") for facet in facets],
        "facets" => facets,
...
@testset "Julia canonicalization owns tracked and adversarial RO-field hashes" begin
    for name in ("sampled-grid.json", "exact-cell-complex.json",
                 "directional-path.json")
        document = _ro_field_contract_fixture(name)
        canonical_data = Backend._canonical_json(document["data"])
        @test document["coverage"]["storage"]["payload_bytes"] ==
            ncodeunits(canonical_data)
        @test document["coverage"]["storage"]["content_sha256"] ==
            bytes2hex(Backend.SHA.sha256(canonical_data))
        @test document["provenance"]["domain_sha256"] ==
            Backend.canonical_hash(document["domain"])
    end

    data_float = _ro_field_contract_fixture("sampled-grid.json")
    data_float["data"]["output_values"][1] = 1.0e-6
    @test_throws ArgumentError Backend.validate_ro_field_document!(data_float)
    canonical_data = Backend._canonical_json(data_float["data"])
    @test occursin("1.0e-6", canonical_data)
    data_float["coverage"]["storage"]["payload_bytes"] =
        ncodeunits(canonical_data)
    data_float["coverage"]["storage"]["content_sha256"] =
        bytes2hex(Backend.SHA.sha256(canonical_data))
    @test Backend.validate_ro_field_document!(data_float) == data_float

    noncanonical_data = replace(canonical_data, "1.0e-6" => "1e-06")
    @test noncanonical_data != canonical_data
    wrong_data_identity = deepcopy(data_float)
    wrong_data_identity["coverage"]["storage"]["payload_bytes"] =
        ncodeunits(noncanonical_data)
    wrong_data_identity["coverage"]["storage"]["content_sha256"] =
        bytes2hex(Backend.SHA.sha256(noncanonical_data))
    @test_throws ArgumentError Backend.validate_ro_field_document!(
        wrong_data_identity)

    domain_float = _ro_field_contract_fixture("sampled-grid.json")
    domain_float["domain"]["fixed_background"][1]["log_value"] = 1.0e-6
    @test_throws ArgumentError Backend.validate_ro_field_document!(domain_float)
    canonical_domain = Backend._canonical_json(domain_float["domain"])
    @test occursin("1.0e-6", canonical_domain)
    domain_float["provenance"]["domain_sha256"] =
        Backend.canonical_hash(domain_float["domain"])
    @test Backend.validate_ro_field_document!(domain_float) == domain_float

    noncanonical_domain = replace(canonical_domain, "1.0e-6" => "1e-06")
    wrong_domain_identity = deepcopy(domain_float)
    wrong_domain_identity["provenance"]["domain_sha256"] =
        bytes2hex(Backend.SHA.sha256(noncanonical_domain))
    @test_throws ArgumentError Backend.validate_ro_field_document!(
        wrong_domain_identity)
end

@testset "exact-cell validator uses dimensionally scaled geometry tolerances" begin
    payload = _ro_field_scaled_exact_payload(1e-6)
    ...
    _, domain_area, length_tolerance, area_tolerance =
        Backend._ro_field_exact_domain_tolerances(bounds)
    left = [
        [-2e-6, -2e-6], [0.0, -2e-6], [0.0, 2e-6], [-2e-6, 2e-6],
    ]
    adjacent = [
        [0.0, -2e-6], [2e-6, -2e-6], [2e-6, 2e-6], [0.0, 2e-6],
    ]
    gapped = [
    ...
    @test Backend._ro_field_polygon_is_convex(
        left; length_tolerance=length_tolerance)
    @test Backend._ro_field_convex_intersection_area(
        left, adjacent; length_tolerance=length_tolerance) <= area_tolerance
    @test domain_area - abs(Backend._ro_field_polygon_signed_area(left)) -
        abs(Backend._ro_field_polygon_signed_area(gapped)) > area_tolerance
    @test Backend._ro_field_convex_intersection_area(
        left, overlapping; length_tolerance=length_tolerance) > area_tolerance

    overlap_payload = deepcopy(payload)
    duplicate = deepcopy(first(overlap_payload["data"]["cells"]))
    duplicate["cell_id"] = "cell-small-scale-overlap"
    duplicate["source_regime_ids"] = Any["regime-small-scale-overlap"]
    duplicate["affine_labels"][1]["label_id"] = "label-small-scale-overlap"
    duplicate["affine_labels"][1]["source_regime_ids"] =
        Any["regime-small-scale-overlap"]
    duplicate["label_order"] = Any["label-small-scale-overlap"]
    push!(overlap_payload["data"]["cells"], duplicate)
    push!(overlap_payload["data"]["cell_order"], duplicate["cell_id"])
    overlap_payload["data"]["regular_candidate_regime_count"] += 1
    overlap_payload["data"]["source_candidate_regime_count"] += 1
    overlap_error = try
        validate_ro_field_payload!(overlap_payload)
        nothing
    catch err
        err
    end
    @test overlap_error isa ArgumentError
    @test occursin("overlap with positive area", sprint(showerror, overlap_error))
end
...
@testset "document and RPB2 payload semantics reject fabricated exact evidence" begin
    ...
    micro_partition = _ro_field_scaled_exact_payload(1e-6)
    @test validate_ro_field_payload!(micro_partition) == micro_partition

    micro_overlap = deepcopy(micro_partition)
    low_overlap = first(micro_overlap["data"]["cells"])
    for vertex_index in (2, 3)
        low_overlap["vertices"][vertex_index][1] += 1e-12
    end
    low_overlap["area"] = Backend._ro_field_polygon_signed_area(
        low_overlap["vertices"])
    overlap_error = try
        validate_ro_field_payload!(micro_overlap)
        nothing
    catch error
        error
    end
    @test overlap_error isa ArgumentError
    if overlap_error isa Exception
        @test occursin(
            "overlap with positive area", sprint(showerror, overlap_error))
    end

    micro_gap = deepcopy(micro_partition)
    low_gap = first(micro_gap["data"]["cells"])
    for vertex_index in (2, 3)
        low_gap["vertices"][vertex_index][1] -= 1e-12
    end
    low_gap["area"] = Backend._ro_field_polygon_signed_area(low_gap["vertices"])
    gap_error = try
        validate_ro_field_payload!(micro_gap)
        nothing
    catch error
        error
    end
    @test gap_error isa ArgumentError
    if gap_error isa Exception
        @test occursin("gap-free exact cells", sprint(showerror, gap_error))
    end

    striped_partition = _ro_field_striped_exact_payload()
    @test validate_ro_field_payload!(striped_partition) == striped_partition

    high_offset_partition = _ro_field_striped_exact_payload(count=1)
    coordinate_shift = 1e8
    for axis in high_offset_partition["domain"]["axes"], key in ("lower", "upper")
        axis["bounds"][key] += coordinate_shift
    end
    for cell in high_offset_partition["data"]["cells"],
        point in cell["vertices"], coordinate in eachindex(point)
        point[coordinate] += coordinate_shift
    end
    for facet in high_offset_partition["data"]["facets"]
        for point in facet["endpoints"], coordinate in eachindex(point)
            point[coordinate] += coordinate_shift
        end
        normal = facet["normal"]
        endpoint = facet["endpoints"][1]
        facet["offset"] = -(normal[1] * endpoint[1] + normal[2] * endpoint[2])
    end
    @test validate_ro_field_payload!(high_offset_partition) == high_offset_partition
    high_offset_bad_metadata = deepcopy(high_offset_partition)
    high_offset_bad_metadata["data"]["facets"][1]["offset"] += 1e-3
    @test_throws ArgumentError validate_ro_field_payload!(high_offset_bad_metadata)

    cumulative_overlap = _ro_field_striped_exact_payload(interface_delta=5e-9)
    cumulative_overlap_error = try
        validate_ro_field_payload!(cumulative_overlap)
        nothing
    catch error
        error
    end
    @test cumulative_overlap_error isa ArgumentError
    if cumulative_overlap_error isa Exception
        @test occursin("cumulative cell overlap",
            sprint(showerror, cumulative_overlap_error))
    end

    cumulative_holes = _ro_field_striped_exact_payload(interface_delta=-5e-9)
    cumulative_hole_error = try
        validate_ro_field_payload!(cumulative_holes)
        nothing
    catch error
        error
    end
    @test cumulative_hole_error isa ArgumentError
    if cumulative_hole_error isa Exception
        @test occursin("gap-free exact cells",
            sprint(showerror, cumulative_hole_error))
    end

    cancelling_errors = _ro_field_striped_exact_payload()
    overlap_width = 5e-9
    hole_width = 1e-8
    cancelling_cells = cancelling_errors["data"]["cells"]
    for vertex_index in (2, 3)
        cancelling_cells[1]["vertices"][vertex_index][1] += overlap_width / 2
        cancelling_cells[2]["vertices"][vertex_index][1] -= hole_width / 2
    end
    for vertex_index in (1, 4)
        cancelling_cells[2]["vertices"][vertex_index][1] -= overlap_width / 2
        cancelling_cells[3]["vertices"][vertex_index][1] += hole_width / 2
    end
    cancelling_error = try
        validate_ro_field_payload!(cancelling_errors)
        nothing
    catch error
        error
    end
    @test cancelling_error isa ArgumentError
    if cancelling_error isa Exception
        @test occursin("combined global overlap/gap budget",
            sprint(showerror, cancelling_error))
    end

    cumulative_facet_gaps = _ro_field_striped_exact_payload(count=1)
    filter!(facet -> facet["facet_id"] != "bottom-1",
        cumulative_facet_gaps["data"]["facets"])
    segment_count = 100
    segment_width = 4.0 / segment_count
    segment_gap = 3e-10
    for index in 1:segment_count
        left = -2.0 + (index - 1) * segment_width +
            (index == 1 ? 0.0 : segment_gap / 2)
        right = -2.0 + index * segment_width -
            (index == segment_count ? 0.0 : segment_gap / 2)
        push!(cumulative_facet_gaps["data"]["facets"],
            _ro_field_test_exact_facet(
                "bottom-segment-$index", "domain_boundary",
                Any[Any[left, -2.0], Any[right, -2.0]],
                ["cell-1"], "axis2_lower"))
    end
    cumulative_facet_gaps["data"]["facet_order"] = Any[
        facet["facet_id"] for facet in cumulative_facet_gaps["data"]["facets"]
    ]
    cumulative_facet_error = try
        validate_ro_field_payload!(cumulative_facet_gaps)
        nothing
    catch error
        error
    end
    @test cumulative_facet_error isa ArgumentError
    if cumulative_facet_error isa Exception
        @test occursin("cumulative uncovered facet length",
            sprint(showerror, cumulative_facet_error))
    end
